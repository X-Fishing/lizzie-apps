...
export async function carregarPrecificacao(force = false) {
  ...
  if (pRes.error || bRes.error) {
    ...
    return null; // migração 0012 não rodada, provavelmente
  }
  precifCache = {
...
const painelSemConfig = (panelId) => {
  document.getElementById(panelId).innerHTML =
    '<div class="empty-state"><div class="empty-icon"><svg class="ico" viewBox="0 0 24 24" aria-hidden="true"><line x1="19" x2="5" y1="5" y2="19"/><circle cx="6.5" cy="6.5" r="2.5"/><circle cx="17.5" cy="17.5" r="2.5"/></svg></div><p>Configuração de precificação não encontrada — rode a migração <b>0012_precificacao.sql</b> no Supabase.</p></div>';
};
...
export function loteSet(i, campo, valor, el) {
  ...
  if (campo === 'pecaBruta') {
    ...
    if (cat && cat.banho_padrao === undefined) {
      toast('A coluna "banho padrão" não existe no banco — rode a migração 0014 no Supabase.');
    }
