...
async function abrirForm(p) {
  ...
  precifCfg = await carregarPrecificacao(); // null = migração 0012 ausente
  const editando = !!p;
  p = p || {};
  ...
  panel().innerHTML = `
    ...
    ${secHeader('Precificação', precifCfg ? '' : 'rode a migração 0012')}
    ${precifCfg ? `
    <div class="form-grid">
    ...
    </div>` : `<div style="font-size:12.5px;color:var(--muted);padding:6px 0 2px">Cálculo de custo/preço indisponível — rode a migração <b>0012_precificacao.sql</b> no Supabase.</div>`}
...
export async function produtoSalvar(id) {
  ...
  // Migração 0012 ausente: salva sem os campos de precificação.
  if (error && /tipo_banho|preco_bruto|precificado|cotacao_usada|desconto_usado|custo_verniz|preco_sugerido|modelo|verniz|banho|peso/i.test(error.message || '') && /column|schema cache/i.test(error.message || '')) {
    console.warn('Colunas de precificação ausentes (rode a migração 0012):', error.message);
    ...
    await gravar();
    if (!error) toast('Salvo sem a precificação — rode a migração 0012 no Supabase.');
  }
  if (error) {
